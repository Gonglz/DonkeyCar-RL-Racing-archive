...
except Exception:  # pragma: no cover - Python 3.6 without dataclasses backport
    def dataclass(cls=None, **_kwargs):
        def wrap(kls):
            fields = list(getattr(kls, "__annotations__", {}).keys())
            defaults = {
            ...
            def __init__(self, *args, **kwargs):
                ...
                if kwargs:
                    raise TypeError(
                        "unexpected keyword argument: " + next(iter(kwargs))
                    )
...
class V17Pilot:
    ...
    def __init__(
        ...
        trt_metadata_path: Optional[str] = None,
        obs_size: int = 128,
        ...
        self.trt_engine_path = (
            os.path.abspath(os.path.expanduser(trt_engine_path))
            if trt_engine_path else ""
        )
        self.trt_metadata_path = (
            os.path.abspath(os.path.expanduser(trt_metadata_path))
            if trt_metadata_path else ""
        )
        if torch is None and not self.trt_engine_path:
            raise ImportError("PyTorch is required for V17Pilot")
        ...
        self.device = (
            ...
            else ("cuda" if use_cuda and torch is not None and torch.cuda.is_available() else "cpu")
        )
    ...
    def _load_trt_actor(self):
        tools_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tools")
        if tools_dir not in sys.path:
            sys.path.insert(0, tools_dir)
        from v17_trt_runtime import V17TensorRTActor

        metadata_path = self.trt_metadata_path
        if not metadata_path:
            metadata_path = os.path.join(os.path.dirname(self.trt_engine_path), "v17_actor_export.json")

        actor = V17TensorRTActor(self.trt_engine_path, metadata_path=metadata_path)
        shape = actor.metadata.get("shape", {})
        if shape:
            self.image_channels = int(shape.get("image_channels", self.image_channels))
            self.obs_size = int(shape.get("obs_size", self.obs_size))
            self.state_dim = int(shape.get("state_dim", self.state_dim))
            self.lidar_dim = int(shape.get("lidar_dim", self.lidar_dim))
            self.lidar_meta_dim = int(shape.get("lidar_meta_dim", self.lidar_meta_dim))
        self.observation_keys = ["image", "state", "lidar", "lidar_meta"]
        self.backend = "TensorRT FP16 actor"
        print(f"V17Pilot: using TensorRT actor engine {self.trt_engine_path}")
        return actor

    def _load_model(self):
        if self.trt_engine_path:
            return self._load_trt_actor()
    ...
    def _build_lidar_obs(self, lidar: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        sector_ranges = lidar.get("sector_ranges")
        sector_valid = lidar.get("sector_valid")
        if sector_ranges is not None and sector_valid is not None:
            try:
                ranges_arr = np.asarray(sector_ranges, dtype=np.float32).reshape(-1)
                valid_arr = np.asarray(sector_valid, dtype=np.float32).reshape(-1)
            except Exception:
                ranges_arr = None
                valid_arr = None
            if ranges_arr is not None and ranges_arr.shape[0] == sectors and valid_arr.shape[0] == sectors:
                return np.concatenate([ranges_arr, valid_arr]).astype(np.float32), self._build_lidar_meta(lidar)
...
def _run_smoke(args: argparse.Namespace) -> None:
    pilot = V17Pilot(
        ...
        trt_metadata_path=args.engine_metadata,
...
def main() -> None:
    ...
    parser.add_argument("--engine", default=None, help="Optional TensorRT actor .engine")
    parser.add_argument("--engine-metadata", default=None, help="Optional TensorRT actor metadata JSON")
