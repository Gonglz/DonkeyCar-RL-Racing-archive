...
- Four simulator workers by default: ws, gt, ws, gt on ports 9091/9093/9095/9097
"""
...
DEFAULT_V18_PORTS: Tuple[int, ...] = (9091, 9093, 9095, 9097)


def build_v18_worker_assignments(
    *,
    env_ids: Optional[List[str]] = None,
    ports: Optional[List[int]] = None,
) -> List[Dict[str, Any]]:
    env_ids = list(DEFAULT_ENV_IDS if env_ids is None else env_ids)
    ports = list(DEFAULT_V18_PORTS if ports is None else ports)
    if not env_ids:
        raise ValueError("env_ids must not be empty")
    for env_id in env_ids:
        if env_id not in SCENE_SPECS:
            raise KeyError(f"Unsupported env_id for V18: {env_id}")
    if len(ports) < len(env_ids) or len(ports) % len(env_ids) != 0:
        raise ValueError(
            "V18 worker ports must contain a balanced number of full scene replicas: "
            f"len(ports)={len(ports)}, len(env_ids)={len(env_ids)}"
        )
    ports = [int(p) for p in ports]
    if len(set(ports)) != len(ports):
        raise ValueError(f"V18 worker ports must be unique, got {ports}")

    assignments: List[Dict[str, Any]] = []
    for worker_idx, worker_port in enumerate(ports):
        scene_idx = worker_idx % len(env_ids)
        env_id = env_ids[scene_idx]
        spec = SCENE_SPECS[env_id]
        assignments.append(
            {
                "worker_idx": int(worker_idx),
                "replica_idx": int(worker_idx // len(env_ids)),
                "scene_idx": int(scene_idx),
                "env_id": str(env_id),
                "scene_key": str(spec["scene_key"]),
                "logging_key": str(spec.get("logging_key", spec["scene_key"])),
...
def _v18_vec_env_mode(worker_assignments: List[Dict[str, Any]]) -> str:
    return "subproc" if len(worker_assignments) > 1 else "dummy"
...
CURRICULUM_PHASES["avoid_static"].update(
    {
        **_V17_AVOID_REWARD_SHAPING,
        ...
        "ws_obstacle_lateral_choices": [0.0, 1.0],
...
CURRICULUM_PHASES["avoid_mixed"].update(
    {
        **_V17_AVOID_REWARD_SHAPING,
        "obstacle_free_prob": 0.10,
        "ws_obstacle_free_prob": 0.10,
        "ws_obstacle_lateral_choices": [0.02, 0.98],
        "ws_obstacle_progress_min": 0.40,
        "ws_obstacle_progress_max": 0.60,
        "obstacle_target_ratios": {"ws": 0.90, "gt": 0.90},
        "reward_overrides_by_logging_key": {
...
CURRICULUM_PHASES["lane_pid_intro"].update(
    {
        ...
        **_V17_PID_OVERTAKE_INTRO,
        ...
        "ws_obstacle_lateral_choices": [0.08, 0.92],
...
def train_v17(
    ...
    save_dir: str = "models/v18_multisim_lidar",
    port: int = 9091,
    obs_size: int = 128,
    augment: bool = False,
    ...
    config_filename: str = "v18_config.json",
    ...
    for env_id in env_ids:
        if env_id not in SCENE_SPECS:
            raise KeyError(f"Unsupported env_id for V18: {env_id}")
    if port_per_scene is None:
        worker_ports = (
            list(DEFAULT_V18_PORTS)
            if list(env_ids) == list(DEFAULT_ENV_IDS)
            else [int(port)]
        )
    else:
        worker_ports = [int(p) for p in port_per_scene]
    worker_assignments = build_v18_worker_assignments(env_ids=env_ids, ports=worker_ports)
    n_workers = len(worker_assignments)
    ...
    print("\n" + "=" * 76)
    print("🚀 DonkeyCar PPO V18 - 4x Multi-Sim LiDAR-First Training")
    print("=" * 76)
    print(f"maps: {env_ids}")
    print(
        "workers: "
        + ", ".join(
            f"{a['worker_idx']}:{a['logging_key']}@{a['port']}"
    ...
    if not _launch_sim:
        ports_to_probe = [int(a["port"]) for a in worker_assignments]
    ...
    dummy_vec_env = DummyVecEnv([lambda: DummyEnv() for _ in range(n_workers)])
    _safe_seed_env(dummy_vec_env, seed, label="dummy_v18_env")
    ...
    if resume_ckpt_path is None and resume_latest:
        resume_ckpt_path = _find_latest_checkpoint(save_dir, name_prefix="v18")
        if resume_ckpt_path is None:
            raise FileNotFoundError(f"No v18 checkpoint found in {save_dir}")
    ...
    def make_env(worker_assignment: Dict[str, Any]):
        worker_idx = int(worker_assignment["worker_idx"])
        worker_env_id = str(worker_assignment["env_id"])
        worker_port = int(worker_assignment["port"])
        worker_logging_key = str(worker_assignment["logging_key"])
        worker_conf = dict(conf)
        worker_conf["port"] = worker_port
        worker_snapshot_dir = os.path.join(
            snapshot_dir,
            f"worker_{worker_idx}_{worker_logging_key}_{worker_port}",
        )
        os.makedirs(worker_snapshot_dir, exist_ok=True)
        return MultiSceneEnvV17(
            env_ids=[worker_env_id],
            conf=worker_conf,
            scene_weights=[1.0],
            ...
            snapshot_dir=worker_snapshot_dir,
            ...
            obstacle_seed=(
                None if obstacle_seed is None else int(obstacle_seed) + worker_idx
            ),
    ...
    def make_env_factory(worker_assignment: Dict[str, Any]):
        assignment = dict(worker_assignment)

        def _make_env():
            return make_env(assignment)

        return _make_env

    env_fns = [make_env_factory(a) for a in worker_assignments]
    if _v18_vec_env_mode(worker_assignments) == "subproc":
        env = SubprocVecEnv(env_fns, start_method="fork")
    else:
        env = DummyVecEnv(env_fns)
    _safe_seed_env(env, seed, label="v18_train_env")
    if curriculum_phase is not None and hasattr(env, "envs"):
    ...
    config = {
        ...
        "sim": {
            ...
            "host": sim_host,
            "port": sim_port,
            "ports": [int(a["port"]) for a in worker_assignments],
            "workers": worker_assignments,
            "vec_env_mode": _v18_vec_env_mode(worker_assignments),
        },
...
def train_v17_auto_curriculum(
    ...
    **train_kwargs: Any,
    ...
    print("\n" + "=" * 76)
    print("🎓 Start V18 Auto Curriculum")
    print("=" * 76)
    ...
    for stage_idx, stage_def in enumerate(AUTO_CURRICULUM_STAGES):
        ...
        stage_result = train_v17(
            ...
            config_filename=f"v18_config_{stage_name}.json",
            **stage_train_kwargs,
        )
    ...
    auto_summary_path = os.path.join(save_dir, "v18_auto_curriculum_summary.json")
    with open(auto_summary_path, "w", encoding="utf-8") as f:
        json.dump(auto_summary, f, indent=2, ensure_ascii=False)

    print("\n" + "=" * 76)
    print("✅ V18 auto curriculum finished")
    print("=" * 76)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="V18: LiDAR-first recurrent PPO with balanced multi-sim workers",
    ...
    parser.add_argument("--save-dir", type=str, default="models/v18_multisim_lidar")
    parser.add_argument("--port", type=int, default=9091)
    parser.add_argument(
        "--ports",
        nargs="+",
        type=int,
        default=list(DEFAULT_V18_PORTS),
        help=(
            "Balanced worker sim ports. With default env order ws,gt and four "
            "ports, workers map to ws,gt,ws,gt on 9091,9093,9095,9097."
        ),
    )
    parser.add_argument(
        "--curriculum-phase",
